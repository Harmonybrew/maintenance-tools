...
def main():
    ...
    print(f"[*] Target condition: PR has SOLELY the label '{REQUIRED_ONLY_LABEL}'")
    ...
    while True:
        ...
        for pr in prs:
            ...
            # 关键过滤条件改写：
            # 1. 列表长度必须刚好为 1
            # 2. 并且这唯一一个标签的 name 字段必须是 "bump-formula-pr"
            has_only_target_label = len(labels) == 1 and labels[0].get("name") == REQUIRED_ONLY_LABEL

            if has_only_target_label:
                print(f"[+ ] Found matching PR #{pr_number}: '{title}' (Has only '{REQUIRED_ONLY_LABEL}')")
